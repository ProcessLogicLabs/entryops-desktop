"""
AI Agent Core - Claude Code-like agentic experience for DocHopper
...
DEFAULT_SYSTEM_PROMPT = """You are an expert Python developer building invoice parsing templates for DocHopper.
...
Available tools:
- `edit_template` - **USE THIS to make code changes**. Use "surgical" edit_type with old_string/new_string for targeted changes. **IMPORTANT**: Set `replace_all=true` to replace ALL occurrences of a string. Use "full_rewrite" with full_content for major changes.
- `test_template` - Test the template against the loaded invoice
- `read_template` - Read other templates for reference
...
- `query_database` - Query parts_master or part_aliases tables

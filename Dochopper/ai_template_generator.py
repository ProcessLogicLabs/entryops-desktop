"""
ai_template_generator.py — Token-efficient AI template generator for OCRMill / DocHopper.
...
def _try_import_base():
    ...
    candidates = [
        ...
        ("SmartExtractor.base_template",    "BaseTemplate"),   # DocHopper
...
# The interface contract described in ONE paragraph (no code pasted)
_INTERFACE_CONTRACT = (
    "Generate a Python class called InvoiceTemplate that inherits from BaseTemplate. "
    "It must implement an `extract(self) -> dict` method that uses regex or string "
    "operations to parse the invoice text stored in `self.pdf_text` (a str attribute "
    "set by the framework) and return a flat dict whose keys are the field names "
    "(e.g. invoice_number, invoice_date, vendor_name, total_amount, line_items). "
    "line_items should be a list of dicts. Include `TEMPLATE_NAME` and `TEMPLATE_VERSION` "
    "class attributes. The class must also provide `get_metadata() -> dict` returning "
    "at minimum {'name': TEMPLATE_NAME, 'version': TEMPLATE_VERSION}. "
    "Output ONLY valid Python — no markdown fences, no explanation."
)

_SYSTEM_FAST = (
    "You are an expert Python developer specialising in invoice parsing. "
    "Generate concise, correct Python template code that extracts structured data "
    "from invoice text using the interface described by the user."
)

_SYSTEM_STAGE1 = (
    "You are an expert at analysing invoice documents. "
    "Identify the invoice structure in 3-5 bullet points (field names, layout quirks, "
    "repeated patterns). Be extremely brief — your notes will feed a code-generation step."
)

_SYSTEM_STAGE2 = (
    "You are an expert Python developer. Using the structural analysis provided, "
    "generate a precise invoice parsing template that extracts exactly the fields identified."
)


def build_fast_prompt(processed_text: str, extra_hint: str = "") -> tuple[str, str]:
    """Return (system_prompt, user_prompt) for the single-call fast mode."""
    hint_line = f"\nExtra context: {extra_hint.strip()}" if extra_hint.strip() else ""
    user = (
        f"Invoice text sample (pre-processed):\n"
        f"---\n{processed_text}\n---\n\n"
        f"{_INTERFACE_CONTRACT}"
        f"{hint_line}"
    )
...
def build_stage1_prompt(processed_text: str) -> tuple[str, str]:
    ...
    user = (
        f"Analyse this invoice sample and list 3-5 key structural observations "
        f"(fields present, how line items are formatted, date/number formats, "
        f"any unusual layout). Be very brief.\n\n"
        f"---\n{processed_text}\n---"
    )
    return _SYSTEM_STAGE1, user


def build_stage2_prompt(processed_text: str, analysis: str, extra_hint: str = "") -> tuple[str, str]:
...
def wrap_template_code(raw_code: str) -> str:
    """Strip markdown fences from AI output and ensure correct base class import.

    Inserts the appropriate `from X import BaseTemplate` line if the AI forgot it,
    or replaces a wrong one. Falls back to the inline stub if no import path is known.
    """
    ...
    # Determine the correct import line
    if _BASE_TEMPLATE_IMPORT:
        mod_path, cls_name = _BASE_TEMPLATE_IMPORT
        import_line = f"from {mod_path} import {cls_name}"
    else:
        # No base class found at runtime — embed the stub
        import_line = None

    # Remove any existing BaseTemplate import so we can insert the right one
    code = re.sub(r"^from\s+\S+\s+import\s+BaseTemplate.*$", "", code, flags=re.MULTILINE)
    code = re.sub(r"^import\s+base_template.*$", "", code, flags=re.MULTILINE)

    header_lines = [
        "# Auto-generated by AITemplateGenerator — edit as needed.",
        "import re",
    ...
    if import_line:
        ...
        header_lines.append("")
        header_lines.append("# BaseTemplate stub (no app base class found at generation time)")
        header_lines.extend(_INLINE_BASE_STUB.splitlines())

    header = "\n".join(header_lines) + "\n\n"

    # If code already starts with imports, don't double-add stdlib ones
    if code.startswith("import re") or code.startswith("# Auto-generated"):
        return code
...
class AIGeneratorThread(QThread):
    ...
    Signals:
        chunk_ready(str)       — streaming text chunk (may arrive many times)
        generation_complete(str) — full generated code (after all stages)
        generation_error(str)  — error message string
        stage_update(str)      — status message, e.g. "Stage 1/2: analysing…"
    """
    ...
    def run(self):
        try:
            if self.fast_mode:
                ...
                system, user = build_fast_prompt(self.processed_text, self.extra_hint)
                raw = self._dispatch(system, user, max_tokens=1500, stream=True)
            else:
                # ---- Two-stage mode ----
                # Stage 1: identify structure (small, ~200 token output)
                self.stage_update.emit("Stage 1/2: Analysing invoice structure…")
                s1_sys, s1_usr = build_stage1_prompt(self.processed_text)
                analysis = self._dispatch(s1_sys, s1_usr, max_tokens=300, stream=False)
                if self._cancelled:
                    return

                # Stage 2: generate code using the analysis
                self.stage_update.emit("Stage 2/2: Generating template…")
                s2_sys, s2_usr = build_stage2_prompt(
                    self.processed_text, analysis, self.extra_hint
                )
                raw = self._dispatch(s2_sys, s2_usr, max_tokens=1500, stream=True)

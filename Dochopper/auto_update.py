"""
Auto-Update Module for DocHopper
...
class UpdateChecker(QThread):
    ...
    def __init__(self, current_version, github_repo=None):
        super().__init__()
        if github_repo is None:
            try:
                from Dochopper._branding import UPDATE_REPO
            except ImportError:
                from _branding import UPDATE_REPO  # type: ignore[no-redef]
        ...
        # Determine major version to filter releases by compatibility
        try:
    ...
    def run(self):
        ...
        try:
            ...
            request = Request(
                self.api_url,
                headers={'User-Agent': 'DocHopper-AutoUpdater'}
            )
...
class UpdateDownloader(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            temp_dir = Path(tempfile.gettempdir()) / "DocHopper_Updates"
...
class AutoUpdateManager:
    """
    Manages the auto-update process for DocHopper.
    ...
    def _on_update_available(self, update_info):
        ...
        text = f"<h3>DocHopper {version} is now available!</h3>"
    ...
    def _on_download_complete(self, installer_path, progress_dialog):
        ...
        msg.setInformativeText(
            "DocHopper will now close and install the update. "

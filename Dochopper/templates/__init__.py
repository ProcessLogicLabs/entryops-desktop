...
def get_shared_templates_folder() -> str:
    ...
    try:
        ...
        if getattr(sys, 'frozen', False):
            import os
            base_dir = Path(os.environ.get('LOCALAPPDATA', Path.home())) / "DocHopper"
        else:
            ...
            for key in ('windows_path', 'linux_path', 'path'):
                if config.has_option('Database', key):
                    ...
                    if p:
                        ...
                        break
        if not db_path:
            db_path = str(base_dir / "dochopper.db")
...
def _load_template_from_file(file_path: Path, module_name: str) -> bool:
    ...
    try:
        ...
        if 'templates' not in sys.modules:
            # If this module was loaded as Dochopper.templates, reuse it
            real_module = sys.modules.get('Dochopper.templates')

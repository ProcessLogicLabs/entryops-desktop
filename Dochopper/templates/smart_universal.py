...
try:
    ...
    parent_dir = current_path.parent if 'Dochopper' in current_path.name else current_path
    if str(parent_dir) not in sys.path:
        sys.path.insert(0, str(parent_dir))
...
try:
    ...
    try:
        from Dochopper.smart_extractor import SmartExtractor, ExtractionResult
    except ImportError:
        SmartExtractor = None

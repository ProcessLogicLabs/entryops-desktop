...
`Dochopper.templates.isf_10_plus_2.ISF10Plus2Template` can consume it
unchanged.
"""
...
def _read_pdf_text(path: Path, db_path: Path | None = None) -> str:
    ...
    to the shared DocHopper OCR engine (Tesseract by default). The OCR config
...
def _ocr_pdf_text(path: Path, db_path: Path | None) -> str:
    """Run the configured DocHopper OCR backend; return '' on any failure."""
    try:
        from Dochopper.ocrmill_processor import _OCRConfigAdapter

...
"""
EntryOps PyInstaller Spec File - Standard Edition
Build command: pyinstaller entryops.spec
"""
...
# EntryOps. Reintroduce only if a future feature needs it.)
_isf_datas = []
_isf_binaries = []
_isf_hiddenimports = []
for _pkg in ('xlrd',):
...
# Get the absolute path to the Entryops directory
entryops_dir = os.path.join(os.path.dirname(os.path.abspath(SPEC)), 'entryops')
...
a = Analysis(
    [os.path.join(entryops_dir, 'entryops.py')],
    pathex=[entryops_dir],
    ...
    datas=[
        ...
        (os.path.join(entryops_dir, 'Resources', 'References', 'SEC232.txt'), 'Resources/References'),
        (os.path.join(entryops_dir, 'Resources', 'References', 'Attachment 2_Auto Parts HTS List.txt'), 'Resources/References'),
        (os.path.join(entryops_dir, 'Resources', 'References', 'parts_master_template.csv'), 'Resources/References'),
        (os.path.join(entryops_dir, 'Resources', 'References', 'tariff_232_import_template.csv'), 'Resources/References'),
        # Templates directory
        (os.path.join(entryops_dir, 'templates'), 'templates'),
        # OCR subpackage (pure Python, but PyInstaller doesn't auto-bundle
        # nested packages that aren't imported at startup).
        (os.path.join(entryops_dir, 'ocr'), 'ocr'),
        # ISF Filing subpackage retired from EntryOps UI in v1.6.1; the
        ...
        # EntryOps installer (no code path consumes them).
...
exe = EXE(
    ...
    icon=os.path.join(entryops_dir, 'Resources', 'icon.ico'),
)

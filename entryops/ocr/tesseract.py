...
class TesseractBackend(OCRBackend):
    ...
    def __init__(self, config: Any):
        ...
        # PSM 6 ("uniform block of text") preserves columnar layout in
        # invoice tables — the default PSM 3 silently drops narrow numeric
        # columns on dense rows. Configurable via billing_settings.
        self._psm = str(config.get('ocr.tesseract.psm', '6') or '6')
    ...
    def _image_to_text(self, img) -> str:
        ...
        config_parts = [f'--psm {self._psm}']
        if self._tessdata:
            config_parts.append(f'--tessdata-dir "{self._tessdata}"')
        kwargs['config'] = ' '.join(config_parts)
        try:
    ...
    def _cache_path(self, pdf_path: Path, dpi: int,
        ...
        tag = hashlib.sha1(
            ...
            f'{dpi}:{",".join(preprocess or [])}:{self._language}:'
            f'psm{self._psm}'.encode()
        ).hexdigest()[:10]

...
def _resolve_overlay_path() -> Optional[Path]:
    ...
    is strictly optional. Reads the table directly (no EntryOps import)
    to avoid a circular dependency at module-load time."""
    try:
        try:
            from entryops.entryops import DB_PATH
        except ImportError:
            from entryops import DB_PATH  # type: ignore[no-redef]
    except Exception:
        return None
    try:
...
def _load_country_codes(db_path: Path) -> Dict[str, str]:
    """Read country_name → ISO 2-letter mapping from the shared EntryOps DB.
...
def _normalize_country(value: str, lookup: Dict[str, str]) -> str:
    ...
    Looks up ``country_codes`` in the shared EntryOps DB; passes through

"""
OCRMill Processing Engine for EntryOps
...
try:
    from entryops.templates import get_all_templates, refresh_templates
    from entryops.templates.bill_of_lading import BillOfLadingTemplate
except ImportError:
...
class _OCRConfigAdapter:
    """Dotted-path config reader for the entryops/ocr package.

    The ocr/*.py files were ported verbatim from OCRMill, which expects
    ``config.get('ocr.enabled', False)``-style lookups. EntryOps's
    ``OCRMillConfig`` doesn't speak that dialect, and OCR preferences
    aren't a top-level concept in EntryOps's settings yet. This adapter
    ...
    DEFAULTS = {
        "ocr": {
            ...
            "tesseract": {
                ...
                "language": "eng",
                "dpi": 300,
                # PSM 6 = "uniform block of text". Beats the Tesseract default
                # (3 = auto with OSD) on dense invoice tables where narrow
                # numeric columns get dropped under auto-segmentation.
                "psm": "6",
...
class ProcessorEngine:
    ...
    def process_and_export(self, items: List[Dict], output_folder: Path,
        ...
        Args:
            ...
            db_path: Path to the EntryOps database
        ...
        try:
            from entryops.ocrmill_enrichment import EnrichmentPipeline
        except ImportError:
        ...
        try:
            from entryops.ocrmill_exporter import ProfileExporter
        except ImportError:
        ...
        if profile is None:
            try:
                from entryops.ocrmill_exporter import DEFAULT_OUTPUT_COLUMN_ORDER
            except ImportError:

# Contributing to DocHopper

Thanks for considering a contribution. DocHopper is a small project with a narrow scope — PDF/Excel extraction for the long tail of "someone is re-keying data from a document into a downstream system" — so this guide is short.

## Where help is most useful

1. **New templates** for specific supplier invoice formats. Drop a `.py` file into `Dochopper/templates/`, inherit from `BaseTemplate`, and it auto-discovers on startup. Use [`sample_template.py`](Dochopper/templates/sample_template.py) as a starting point.
...
- Bump the version in [`pyproject.toml`](pyproject.toml) and [`Dochopper/version.py`](Dochopper/version.py) `__fallback_version__` together — they must agree.
...
Open an issue at https://github.com/ProcessLogicLabs/dochopper/issues with:

- DocHopper version (Help → About, or `python -c "from Dochopper.version import get_version; print(get_version())"`)
